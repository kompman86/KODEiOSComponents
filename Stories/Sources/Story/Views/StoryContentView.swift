...
public class StoryContentView: UIStackView {
  // MARK: - Properties
  
  public var onDidTapActionButton: (() -> Void)?
  ...
  public func configure(with viewModel: StoryContentViewModelProtocol) {
    ...
    setupActionButton()
    if let actionButtonProperties = viewModel.actionButtonProperties {
      actionButton.configure(with: actionButtonProperties)
      actionButton.isHidden = false
    }
    actionButton.snp.remakeConstraints { make in
      make.height.equalTo(viewModel.actionButtonHeight)
    }
