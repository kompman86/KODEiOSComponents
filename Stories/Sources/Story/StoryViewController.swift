...
public protocol StoryViewControllerDelegate: class {
...
public class StoryViewController: UIViewController {
  ...
  weak public var delegate: StoryViewControllerDelegate?
  ...
  private let loadingErrorView = StoryLoadingErrorView()
  private let activityIndicatorView = UIActivityIndicatorView()
  
  private var hasLoadingError: Bool = false

  var currentStoryPreviewImageURLString: String? {
    return viewModel.storyPreviewURL?.absoluteString
  }
  
  // MARK: - Init
  
  public init(viewModel: StoryViewModelProtocol) {
  ...
  private func setupLoadingErrorView() {
    ...
    loadingErrorView.isHidden = true
    loadingErrorView.onDidRequestToRefresh = { [weak self] in
      self?.reloadImageView()
    }
    loadingErrorView.configure(with: viewModel.storyLoadingErrorViewModel)
    loadingErrorView.snp.removeConstraints()
    loadingErrorView.setEqualToSuperviewConstraints(from: viewModel.loadingErrorViewConstraints)
  }
  ...
  private func setupGestures() {
    ...
    createSwipeGesture(withDirection: .down)
...
extension StoryViewController {
  ...
  @objc private func didSwipeStory(_ gestureRecognizer: UISwipeGestureRecognizer) {
    ...
    case .down:
      delegate?.storyViewControllerDidFinish(self)
    case .left:
      showNextStory()
    case .right:
      showPreviousStory()
...
public extension StoryViewController {
  ...
  func prepareForPresenting() {
    storyControlView.changeProgressBarState(.reset)
    imageView.setImage(with: viewModel.currentImageURL)
  }
...
private extension StoryViewController {
  ...
  func configureAppearanceAfterLoading(hasError: Bool, error: NetworkError? = nil) {
    ...
    if let error = error {
      loadingErrorView.update(for: error)
    }
  ...
  func configureStoryAfterLoading(image: UIImage?) {
    ...
    storyControlView.changeProgressBarState(.start(viewModel.storyFrameDuration ?? 0))
  }
